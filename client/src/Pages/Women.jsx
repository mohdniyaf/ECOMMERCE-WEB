...
function Men() {
  ...
  useEffect(() => {
    ...
    const fetchShirtData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/user/products/WOMEN/Shirt`);
    ...
    const fetchPantData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/user/products/WOMEN/Pant`);
